  ...
  template: `
    <div [class]="containerClasses()" [title]="name()">
      @if (src() && !hasError()) {
        ...
        <span class="font-semibold text-slate-700 dark:text-slate-200 uppercase select-none">
          {{ initials() }}
        </span>
      }
    </div>
...
export class AvatarComponent {
  ...
  containerClasses = computed(() => {
    const base =
      'relative flex items-center justify-center rounded-full bg-slate-200 dark:bg-slate-700 ring-2 ring-white dark:ring-slate-800 overflow-hidden shrink-0 shadow-sm';
